...
class DecisionExecutionBridge:
    ...
    def _make_id(self, row: pd.Series) -> str:
        base = '|'.join([
            str(row.get('Ticker', '')),
            str(row.get('Action', '')),
            str(row.get('Reference_Price', '')),
            str(row.get('Target_Qty', '')),
    ...
    def build(self):
        ...
        for i, row in df.iterrows():
            ...
            idem = self._make_id(df.loc[i])
            idempotency_keys.append(idem)
            client_order_ids.append(f"CL-{datetime.now().strftime('%Y%m%d')}-{idem}")
        ...
        df['Order_Type'] = 'LIMIT'
        df['Time_In_Force'] = 'DAY'
        df['Session'] = 'REGULAR'
        df['Idempotency_Key'] = idempotency_keys
        df['Client_Order_ID'] = client_order_ids
        df.to_csv(self.output_path, index=False, encoding='utf-8-sig')
        watchlist = df[['Ticker', 'Action', 'Reference_Price', 'Target_Qty', 'MarketRuleReason', 'SizingReason', 'Client_Order_ID']].copy()
