    ...
    def reconcile(self, local_orders, broker_orders, local_fills, broker_fills, local_positions, broker_positions, local_cash, broker_cash):
        ...
        for oid, row in local_map.items():
            if oid not in broker_map:
                ...
                ledger.record_reconciliation_issue({'order_id': oid, 'direction_bucket': lane, 'issue_type': 'missing_at_broker'})
                continue
            if normalize_key(str(row.get('status', ''))) != normalize_key(str(broker_map[oid].get('status', ''))):
                ...
                lane_issue_breakdown[lane] += 1
                ledger.record_reconciliation_issue({'order_id': oid, 'direction_bucket': lane, 'issue_type': 'status_mismatch'})
        for oid, row in broker_map.items():
            if oid not in local_map:
                lane = _lane(row)
                issues.append({'order_id': oid, 'lane': lane, 'type': 'orphan_broker_order'})
                lane_issue_breakdown[lane] += 1
                ledger.record_reconciliation_issue({'order_id': oid, 'direction_bucket': lane, 'issue_type': 'orphan_broker_order'})
        payload = {'generated_at': now_str(), 'status': 'reconciled', 'order_issue_count': len(issues), 'issues': issues, 'lane_issue_breakdown': dict(lane_issue_breakdown), 'cash_diff': float((broker_cash or 0) - (local_cash or 0)), 'directional_order_mix': dict((lane, c) for lane, c in lane_issue_breakdown.items()), 'directional_fill_mix': {}}
