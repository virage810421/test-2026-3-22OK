...
def ensure_core_tables(cursor):
    ...
    # 中文相容檢視：讓舊版若仍把 monthly_revenue_simple 映射成 [月營收簡表] 時不致找不到物件。
    cursor.execute("""
    IF OBJECT_ID(N'dbo.月營收簡表', N'V') IS NULL
    BEGIN
        EXEC(N'CREATE VIEW dbo.[月營收簡表] AS
            SELECT [Ticker SYMBOL], [公司名稱], [產業類別], [產業類別名稱], [資料年月日], [單月營收年增率(%)], [更新時間]
            FROM dbo.monthly_revenue_simple')
    END
    """)
