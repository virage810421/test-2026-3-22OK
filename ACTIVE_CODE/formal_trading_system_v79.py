...
class FormalTradingSystemV79:
    ...
    def boot(self):
        log('=' * 60)
        log('🚀 啟動 正式交易主控版_v79')
        log('📈 v79 non-broker massive upgrade：bootstrap plan + paper watchlist + fairer readiness scoring：ON')
    ...
    def run(self):
        ...
        with self.progress.stage(1, '訓練治理檢查'):
            self.guard.run('training_orchestrator','訓練治理檢查',lambda: self.training_orchestrator.maybe_execute(),fallback_fn=self._fallback_runtime_json('training_orchestrator.json',{'status':'fallback_stub','dataset':{'exists':False},'models':{'all_required_present':False},'training_readiness_pct':0}))
        with self.progress.stage(2, '決策執行橋接'):
            self.guard.run('decision_execution_bridge','決策執行橋接',lambda: self.decision_execution_bridge.build(),fallback_fn=self._fallback_runtime_json('decision_execution_bridge.json',{'status':'fallback_stub','rows_with_price':0,'rows_with_qty':0,'rows_market_rule_passed':0,'execution_readiness_pct':0}))
        with self.progress.stage(3, '完成度缺口報告'):
            self.guard.run('completion_gap_report','完成度缺口報告',lambda: self.completion_gap_report.build(),fallback_fn=self._fallback_runtime_json('completion_gap_report.json',{'status':'fallback_stub','headline':{'completion_excluding_real_broker_pct':0,'remaining_major_blocks_excluding_real_broker':3}}))
        with self.progress.stage(4, '完整進度報告'):
            self.guard.run('progress_full_report','完整進度報告',lambda: self.progress_full_report.build(),fallback_fn=self._fallback_runtime_json('progress_full_report.json',{'status':'fallback_stub'}))
        ...
        training_report = self._load_json(PATHS.runtime_dir / 'training_orchestrator.json')
        ...
        manifest = {
            ...
            'rows_watchlist_ready': exec_report.get('rows_watchlist_ready', 0),
            'execution_readiness_pct': exec_report.get('execution_readiness_pct', 0),
            'training_readiness_pct': training_report.get('training_readiness_pct', 0),
        ...
        manifest_path = PATHS.runtime_dir / 'run_manifest_v79.json'
        manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding='utf-8')
        log('============================================================')
        log(f"✅ 主控執行完畢｜{render_progress_bar(1.0)}｜總耗時 {total_elapsed}s")
        log(f"📦 execution bridge rows_with_price={exec_report.get('rows_with_price', 0)} | rows_with_qty={exec_report.get('rows_with_qty', 0)} | passed={exec_report.get('rows_market_rule_passed', 0)} | watchlist={exec_report.get('rows_watchlist_ready', 0)}")
        log(f"🧠 training_readiness={training_report.get('training_readiness_pct', 'n/a')}% | ⚙️ execution_readiness={exec_report.get('execution_readiness_pct', 'n/a')}%")
