...
TABLE_KEY = 'monthly_revenue_simple'
TABLE_NAME = sql_table(TABLE_KEY)
...
def _sql_literal_name(full_name):
    return str(full_name).replace('[', '').replace(']', '')


def _object_exists(cursor, full_name, object_type='U'):
    literal = _sql_literal_name(full_name)
    cursor.execute("SELECT OBJECT_ID(?, ?)", literal, object_type)
    row = cursor.fetchone()
    return bool(row and row[0])


def _ensure_monthly_revenue_table(cursor):
    """確保月營收簡表實體表存在。

    舊版本 fts_sql_table_name_map 會把 monthly_revenue_simple 映射成中文表名，
    但 db_setup.py 建立的是 dbo.monthly_revenue_simple。
    這裡做雙保險：
    1. 優先使用英文實體表。
    2. 若表不存在，直接在 ETL 端補建，避免主線 daily/boot 卡死。
    """
    physical_table = 'dbo.monthly_revenue_simple'
    if _object_exists(cursor, physical_table, 'U'):
        return sql_table('monthly_revenue_simple')
...
def write_to_sql(df):
    ...
    with pyodbc.connect(DB_CONN_STR) as conn:
        cursor = conn.cursor()
        table_name = _ensure_monthly_revenue_table(cursor)
        count = 0
